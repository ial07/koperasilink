...
const villages = [
  ...
  },
  {
    ...
    district: 'Rejang Lebong',
    latitude: -3.3598918,
    longitude: 102.5735968,
  },
  {
    name: 'Air Putih Kali Bandung',
    subdistrict: 'Selupu Rejang',
    district: 'Rejang Lebong',
    latitude: -3.4082407,
    longitude: 102.6292529,
  },

  // Curup Tengah
  {
    name: 'Talang Rimbo Baru',
    subdistrict: 'Curup Tengah',
    district: 'Rejang Lebong',
    latitude: -3.4778894,
    longitude: 102.531911,
  },
  {
    name: 'Talang Rimbo Lama',
    subdistrict: 'Curup Tengah',
    district: 'Rejang Lebong',
    latitude: -3.4798,
    longitude: 102.5339,
  ...
  {
    name: 'Sukaraja',
    subdistrict: 'Curup Timur',
    district: 'Rejang Lebong',
    latitude: -3.4624861,
    longitude: 102.537452,
  },
  {
    name: 'Karang Anyar',
    subdistrict: 'Curup Timur',
    district: 'Rejang Lebong',
    latitude: -3.4559152,
    longitude: 102.5303959,
  },
  {
    name: 'Air Meles Bawah',
    subdistrict: 'Curup Timur',
    district: 'Rejang Lebong',
    latitude: -3.4637067,
    longitude: 102.5429587,
  },
  {
    name: 'Talang Ulu',
    subdistrict: 'Curup Timur',
...
const villageInventory: Array<{
  ...
  {
    villageName: 'Kali Padang',
    commodityName: 'Pisang',
    stock: 400,
    capacity: 500,
    price: 8000,
    demand: 150,
...
async function main() {
  ...
  const categories = ['vegetables', 'fruits', 'spices', 'grains'];
  const dbCategoriesMap = new Map<string, CommodityCategory>();
  for (const cat of categories) {
    const created = await prisma.commodityCategory.create({
      data: {
        name: cat,
        description: `Standardized ${cat} category`,
      },
    });
    dbCategoriesMap.set(cat, created);
    console.log(`  ✅ Category: ${cat}`);
  }

  const dbVillagesMap = new Map<string, Village>();
  ...
  const dbCommoditiesMap = new Map<string, Commodity>();
  for (const c of commodities) {
    ...
    const category = dbCategoriesMap.get(c.category);
  ...
  for (const inv of villageInventory) {
    ...
    if (!village || !commodity) {
      console.warn(
        `  ⚠️  Skipping: ${inv.villageName} / ${inv.commodityName} not found`,
      );
      continue;
    }
    ...
    console.log(
      `  📦 ${village.name} → ${inv.commodityName}: ${inv.stock}/${inv.capacity} (${pct}%)`,
    );
  }
  ...
  for (const inv of villageInventory) {
    ...
    for (let m = 0; m < 6; m++) {
      ...
      // const invProgress = 1 - progress; // unused
      ...
      if (isSurplus) {
        ...
        historicalStock = Math.round(
          startStock + (currentStock - startStock) * progress,
        );
      } else if (isShortage) {
