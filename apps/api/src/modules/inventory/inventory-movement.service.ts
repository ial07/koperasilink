...
export class InventoryMovementService {
   ...
   */
  async createMovement(params: CreateMovementParams, tx?: Prisma.TransactionClient) {
      ...
      switch (params.type) {
        ...
        case MovementType.OUT:
          if (currentStock < params.quantity) {
            throw new BadRequestException(`Insufficient stock. Current: ${currentStock}, Requested: ${params.quantity}`);
          }
          currentStock -= params.quantity;
          break;
        case MovementType.ADJUSTMENT:
          ...
          throw new ConflictException('Use IN or OUT for movements. For full adjustment, use recalculateStock helper.');
      }
   ...
   */
  async adjustStock(inventoryId: string, targetStock: number, notes?: string, createdBy?: string) {
  ...
  async getMovements(inventoryId: string, query?: { limit?: number; page?: number }) {
